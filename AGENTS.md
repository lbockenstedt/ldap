...
**LDAP directory module.** Wraps an OpenLDAP/389-DS-style `slapd` and surfaces OU/user/group CRUD, group membership, password reset, rename and unified user/computer search in the WebUI's **Directory** view.

- **Repo:** `github.com/lbockenstedt/ldap`
- **Module type:** `module_type = "directory"`
- **Canonical docs:** [`lm/docs/ldap.md`](../lm/docs/ldap.md) *(in the `lm` repo — the master registry)*
...
`src/ldap_spoke.py` (spoke), `src/ldap_manager.py` (LDAP operations), `src/ldap_dn.py` (DN
handling), `src/ldif_template.py` + `base_structure.ldif` (schema seeding),
`src/replication.py`, `src/entra_ropc_auth.py` (Entra ROPC — see `lm/docs/entra-sso.md`).

## ldap-specific gotchas

- **Two install modes, mirroring `netbox/install.sh`.** By default it installs the *spoke* that manages an LDAP server at `--server-url` (local or remote) **without provisioning one**.
- **`--hub auto` must be passed explicitly** here to get auto-discovery — unlike most modules, where omitting `--hub` implies it.
- This is a **cert distribution target**: the hub brokers a TLS cert install from the `le` spoke via `INSTALL_CERT`.
