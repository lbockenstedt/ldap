...
`python3 -m src.main` (`LdapControlPlane`); spoke `LdapSpoke(BaseSpoke)`. systemd `lm-ldap.service` (`After=network.target slapd.service`, `User=root` for `INSTALL_CERT`). Installer `install_ldap.sh` (pre-seeds slapd debconf `slapd/domain string lm.local`, backend MDB, installs `slapd ldap-utils python3-pip python3-venv git curl jq libldap2-dev libsasl2-dev`, writes `.env` + unit; does **not** auto-apply `base_structure.ldif` — that file is a reference template of the People/Groups OUs + an example `admin_user`, apply it manually with `ldapadd` if you want that seed structure).
...
`SPOKE_ID`, `SPOKE_SECRET`, `HUB_SECRET`, `HUB_URL`, `LDAP_ADMIN_DN`, `LDAP_ADMIN_PW` (required to bind; warned if empty), `LDAP_BASE_DN` (canonical everywhere — debconf domain, `.env`, and the templated LDIF), `LDAP_SERVER_URL` (`ldap://localhost:389`). Mirror-mode: `LDAP_SERVER_ID` (int `1`|`2`), `LDAP_MIRROR_PEERS` (JSON list of peer `ldap(s)://` URLs). Entra ROPC: `ENTRA_TENANT_ID`, `ENTRA_CLIENT_ID`, `ENTRA_CLIENT_CERT` (path), `ENTRA_CLIENT_KEY` (path), `ENTRA_ROPC_SCOPE` (default `openid`) — read by `src/entra_ropc_auth.py` per bind. All are `.env`-written by the installer and re-writable via `UPDATE_CONFIG`.

## Install flags

`install_ldap.sh` has **two modes**:

- **(default) spoke** — `--hub` (accepts a bare IP/host, normalized to `wss://<host>:443`; default `auto` = mDNS/DNS discovery), `--id`/`--name`, `--secret`, `--hub-secret`, `--server-url`, `--all-prereqs` (no-op). Installs the directory spoke that manages a server (local or remote); does NOT install `slapd`. Only passes `--secret`/`--hub-secret` when non-empty (avoid argparse abort); an empty/default secret is dropped so the spoke connects unauthenticated and awaits WebUI approval.
- **`--infra-only` server** — provisions `slapd` fully zero-CLI (canonical base DN, auto admin password, base structure applied, self-signed TLS/LDAPS, optional mirror-mode, Entra ROPC SASL pass-through). Post-`--infra-only` args (the hub's `ldap-server` deploy role passes exactly these): `--base-dn` (derived from host DNS domain if omitted → `dc=lm,dc=local` fallback), `--admin-dn` (default `cn=admin,<base-dn>`), `--admin-pw` (auto-generated if omitted), `--server-id <1|2>`, `--peer <ldap-url>` (repeatable → `LDAP_MIRROR_PEERS`), `--entra-tenant`/`--entra-client`/`--entra-cert`/`--entra-key`/`--entra-scope`, `--server-url`. Idempotent + non-interactive; base DN is fixed at first install. Invoked via `curl -sSL … | bash -s -- --infra-only <args>`.

## Key commands / handlers (`ldap_spoke.handle_command`)

**Legacy/base-level:** `GET_VERSION`, `UPDATE_CONFIG` (re-init `LdapManager`; also persists Entra keys to `.env` and re-applies mirror-mode `cn=config` when `LDAP_SERVER_ID`+`LDAP_MIRROR_PEERS` present), `LIST_OUS`, `CREATE_OU`, `UPDATE_OU` (modrdn rename), `LIST_USERS`, `CREATE_USER` (`inetOrgPerson`; `secrets.token_urlsafe(16)` random password if none, returned; stored as `{SSHA}` not cleartext), `UPDATE_USER`, `LIST_GROUPS`, `CREATE_GROUP` (`groupOfNames` seeded with base DN member), `UPDATE_GROUP`, `ADD_USER_TO_GROUP`, `REMOVE_USER_FROM_GROUP`, `SET_PASSWORD`, `DELETE_ENTITY`, `SEARCH_USERS`, `LDAP_MIGRATE_TENANT` (re-home entries from source base DN to target base DN), `INSTALL_CERT` (hub-brokered LE cert → `/etc/ldap/tls` + `ldapmodify -Y EXTERNAL` on `cn=config` + `systemctl restart slapd`; root-only).

**Tenant-scoped (`LDAP_*` contract; TENANT == OU, 1:1, case-insensitive):** `LDAP_PROVISION_TENANT_OU {tenant_slug}` (idempotent, case-insensitive — reuses an existing OU in any case; creates `ou=<slug>,<base>` + child `ou=users`/`ou=groups`; returns OU DN), `LDAP_CREATE_USER {tenant_slug, uid, attrs, auth_mode:"entra"|"local", upn?, password?}` (entra → `userPassword {SASL}<upn>`; local → `{SSHA}`), `LDAP_UPDATE_USER`, `LDAP_DELETE_USER`, `LDAP_SET_PASSWORD` (local only — refuses `{SASL}` Entra users), `LDAP_CREATE_GROUP`, `LDAP_ADD_MEMBER`, `LDAP_REMOVE_MEMBER`, `LDAP_LIST_USERS {tenant_slug}`, `LDAP_LIST_GROUPS {tenant_slug}`, `LDAP_GET_USER_GROUPS {uid, tenant_slug}` (memberOf ∪ reverse member search — for hub RBAC). Absent `tenant_slug` = base level (back-compat). Slugs are canonical-lower-cased (`ldap_dn.canonical_slug`) so "LRB" == "lrb" resolve to one OU. DN construction/escaping via the pure `src/ldap_dn.py`.

## Key files

`src/main.py`, `src/ldap_spoke.py`, `src/ldap_manager.py` (~300 lines, whole `LdapManager`), `install_ldap.sh`, `base_structure.ldif` (reference OU/user template, not auto-applied), `tests/test_install_cert.py` (INSTALL_CERT unit tests, stubs python-ldap), `README.md`, `.env.template`, `requirements.txt`, `VERSION`.
...
- **Password generation** — `create_user` never hardcodes a password (`secrets.token_urlsafe(16)` default; returns it). The password is stored `{SSHA}`-hashed (`LdapManager._ssha`) because slapd does **not** hash a plain `add`/`modify` of `userPassword` (only the Password-Modify ext-op does) — so hashing before store avoids cleartext passwords at rest.
...
- **DN/filter injection guards** — DN RDN components are escaped via `ldap.dn.escape_dn_chars` (`_escape_rdn`) and search input is RFC-4515 escaped (`\` → `\5c`, `*` → `\2a`, `(` → `\28`, `)` → `\29`). The spoke binds as admin, so this is security-critical.
...
- **Command flow.** Every WebUI action (list/create/update/delete an OU, user, or group; add/remove group membership; reset a password; rename an entry) becomes one `LdapSpoke.handle_command` call, which runs the matching `LdapManager` method. Because `python-ldap` calls are synchronous/blocking, each one runs in a worker thread via `asyncio.to_thread` — this keeps the spoke's heartbeat to the hub alive even if the directory server is slow or unreachable. Without that, a stalled bind would freeze the spoke's event loop, queue every other command behind it, and get the spoke disconnected as unresponsive. `INSTALL_CERT` is the exception: it is async-native (subprocess + filesystem writes), not `to_thread`-wrapped.
- **Connection hygiene.** Read/list operations (`list_ous`, `list_users`, `list_groups`) and the health check (`check_connection`) use the leak-free `LdapManager._conn()` context manager, which opens a fresh bind and always unbinds in `finally` — even on error. Write operations (`create_*`, `update_*`, `delete_entity`, group membership, `set_password`) still call the older `_get_connection()`, which binds but does **not** explicitly unbind. This is a residual gap (slapd reaps idle sockets, so it hasn't surfaced at lab scale), not a fully-applied fix — worth revisiting before a long-running high-throughput deployment.
...
- **"INSTALL_CERT reports ldapmodify error 80."** slapd couldn't read the private key — the key file isn't chowned to the slapd user (Debian: `openldap`), or the file mode is too tight. The spoke chowns the key best-effort and logs a warning if the user is missing/non-standard; confirm `LDAP_SLAPD_USER` matches your distro's slapd user.
