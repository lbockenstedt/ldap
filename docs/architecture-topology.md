...
**Why an agent and its roles are SEPARATE entries in the WebUI.** This is expected, not a bug. The **parent agent** shows under **Agents** (as `module_type="agent"`), and **each role** shows as its **own Spoke** of the matching module type. So one agent hosting three roles produces **four** entries: the agent plus `…-firewall`, `…-ipam`, `…-dns`. If you're wondering "why do I have more spokes than machines," it's because each role is its own sub-spoke connection.
